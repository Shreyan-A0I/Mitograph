...
def main():
    ...
    # [phylop_score, is_pathogenic, is_likely_pathogenic, is_benign,
    #  is_likely_benign, is_vus, sin_pos, cos_pos, apogee_score, mitotip_score]
    n_var_feats = 10
    ...
    for i, (nid, data) in enumerate(variant_nodes):
        # PhyloP score (may be NaN — will impute later)
        ...
        # Clinical significance one-hot
        clin_sig = str(data.get('clinical_significance', '')).lower()
        if 'pathogenic' in clin_sig and 'likely' not in clin_sig:
            var_features[i, 1] = 1.0  # is_pathogenic
        if 'likely pathogenic' in clin_sig:
            var_features[i, 2] = 1.0  # is_likely_pathogenic
        if 'benign' in clin_sig and 'likely' not in clin_sig:
            var_features[i, 3] = 1.0  # is_benign
        if 'likely benign' in clin_sig:
            var_features[i, 4] = 1.0  # is_likely_benign
        if 'uncertain' in clin_sig or 'vus' in clin_sig:
            var_features[i, 5] = 1.0  # is_vus

        # Circular positional encoding
        pos = data.get('pos', 1)
        sin_pos, cos_pos = circular_positional_encoding(pos)
        var_features[i, 6] = sin_pos
        var_features[i, 7] = cos_pos

        # APOGEE score (optional, NaN → 0)
        apogee = data.get('apogee_score', float('nan'))
        if not math.isnan(apogee):
            var_features[i, 8] = apogee

        # MitoTIP score (optional, NaN → 0)
        mitotip = data.get('mitotip_score', float('nan'))
        if not math.isnan(mitotip):
            var_features[i, 9] = mitotip
